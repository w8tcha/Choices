...
if (withDeclarations) {
  ['public/types', 'public/assets/scripts'].forEach((p) => {
    if (fs.existsSync(p)) {
  ...
  const config = {
    ...
    plugins: [
      ...
      typescript({
        ...
        // Declaration files are generated by a separate `tsc --emitDeclarationOnly` pass (js:build:types)
        // instead of through this plugin: @rollup/plugin-typescript now requires declarationDir/outDir to
        // live inside the directory of the Rollup output file, which conflicts with emitting types to
        // public/types while bundling into public/assets/scripts. See https://github.com/rollup/plugins/issues/1495
        declaration: false,
        declarationMap: false,
        // Without an explicit outDir, the plugin creates a temp outDir for allowJs support (entry.js is JS)
        // that then fails its own same-directory-as-output validation, so pin it to the bundle's directory.
