...
const OUTPUT_TYPES = (
  process.env.OUTPUT_TYPES ||
  Object.keys(outputTypes)
    .filter((k) => outputTypes[k].default)
    .join(',')
).split(',');
...
if (withDeclarations) {
  ['public/types/src', 'public/assets/scripts'].forEach((p) => {
    if (fs.existsSync(p)) {
...
const candidateBuilds = process.env.TARGET ? builds.filter((build) => build.name === process.env.TARGET) : builds;
